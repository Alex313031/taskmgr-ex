    ...
    )
{
    ...
    pOldProcInfo = FindProcInArrayByPID(
                       (CPtrArray *)pParms->pProcPage->m_pProcArray,
                       dwThreadId);
...
HRESULT CProcInfo::SetDataWowTask(__int64  TotalTime,
                                  DWORD          dwThreadId,
                                  CHAR *         pszFilePath,
                                  __int64  uPassCount,
                                  CProcInfo *    pParentProcInfo,
                                  ...
                                  BOOL           fUpdateOnly)
{
    ...
    if ( NT_SUCCESS(Status) )
    {
        ...
        {
            __int64 TimeDelta, Time;

            Time = (LONGLONG)(ullUser + ullKernel);

            TimeDelta = Time - m_CPUTime;
            ...
            if (pParentProcInfo->m_DisplayCPUTime > m_CPUTime)
            {
                pParentProcInfo->m_DisplayCPUTime -= m_CPUTime;
            }
            else
            {
                pParentProcInfo->m_DisplayCPUTime = 0;
            }

            m_DisplayCPUTime = m_CPUTime;
        }
...
HRESULT CProcInfo::SetData(__int64                TotalTime, 
                           PSYSTEM_PROCESS_INFORMATION  pInfo, 
                           __int64                uPassCount,
                           CProcPage *                  pProcPage,
                           BOOL                         fUpdateOnly)
{
    ...
    if (pInfo->UserTime + pInfo->KernelTime < m_CPUTime)
    {
    ...
    if (TimeDelta)
    {
        m_CPUTime = m_DisplayCPUTime = Time;
        m_fDirty_COL_CPUTIME = TRUE;
    }
    ...
    if (m_IoReadOperCount != pInfo->ReadOperationCount)
    {
        m_fDirty_COL_READOPERCOUNT = TRUE;
        m_IoReadOperCount = pInfo->ReadOperationCount;
    }

    if (m_IoWriteOperCount != pInfo->WriteOperationCount)
    {
        m_fDirty_COL_WRITEOPERCOUNT = TRUE;
        m_IoWriteOperCount = pInfo->WriteOperationCount;
    }

    if (m_IoOtherOperCount != pInfo->OtherOperationCount)
    {
        m_fDirty_COL_OTHEROPERCOUNT = TRUE;
        m_IoOtherOperCount = pInfo->OtherOperationCount;
    }

    if (m_IoReadXferCount != pInfo->ReadTransferCount)
    {
        m_fDirty_COL_READXFERCOUNT = TRUE;
        m_IoReadXferCount = pInfo->ReadTransferCount;
    }

    if (m_IoWriteXferCount != pInfo->WriteTransferCount)
    {
        m_fDirty_COL_WRITEXFERCOUNT = TRUE;
        m_IoWriteXferCount = pInfo->WriteTransferCount;
    }

    if (m_IoOtherXferCount != pInfo->OtherTransferCount)
    {
        m_fDirty_COL_OTHERXFERCOUNT = TRUE;
        m_IoOtherXferCount = pInfo->OtherTransferCount;
    }
    ...
    if (g_Options.m_fShow16Bit)
    {
             ...
             ! m_fWowProcessTested)
        {
            ...
            if ( ( m_pszImageName != NULL ) && ( ! _wcsicmp(m_pszImageName, TEXT("ntvdm.exe")) ) )
            {
                ...
                WowTaskCallbackParms.TotalTime = TotalTime;
                WowTaskCallbackParms.TimeLeft = TimeDelta;
...
HRESULT CProcPage::UpdateProcListview ()
{
    ...
    int iCurrent, iCurrListViewItem;
    for (iCurrent = 0, iCurrListViewItem = 0;
...
HRESULT CProcPage::UpdateProcInfoArray()
{
    ...
    // overflow __int64 updates, I'll already be long gone, so don't bug me.
    //

    static __int64 uPassCount = 0;
    ...
    g_PhysMEMMax = SysInfoTemp.m_dwPhysicalMemory;
    ...
    do
    {
        ...
        pOldProcInfo = FindProcInArrayByPID((CPtrArray *)m_pProcArray, PtrToUlong(pCurrent->UniqueProcessId));
        if (pOldProcInfo)
        {
            if (pOldProcInfo->GetCPUTime() > pCurrent->KernelTime + pCurrent->UserTime)
            {
            ...
            {
                LastTotalTime += pOldProcInfo->GetCPUTime();
            }
        }

        TotalTime += pCurrent->KernelTime + pCurrent->UserTime;
...
next:
    ...
    __int64 TimeDelta;
    TimeDelta = TotalTime - LastTotalTime;
    ...
    do
    {
        ...
        CProcInfo * pProcInfo;
        pProcInfo = FindProcInArrayByPID((CPtrArray *)m_pProcArray, PtrToUlong(pCurrent->UniqueProcessId));
    ...
    while (i < m_pProcArray->GetSize())
    {
        ...
        if (pProcInfo->m_uPassCount != uPassCount)
        {
            delete pProcInfo;
    ...
    ResortArray((CPtrArray **)&m_pProcArray);
    uPassCount++;
    ...
    case LVN_GETDISPINFO:
        {
            ...
            if (plvitem->mask & LVIF_TEXT)
            {
                ...
                case COL_CPUTIME:
                {
                    ...
                    RtlTimeToElapsedTimeFields ( (__int64 *)&(pProcInfo->m_DisplayCPUTime), &TimeOut);
...
BOOL CProcPage::IsSystemProcess(DWORD pid, CProcInfo * pProcInfo)
{
    ...
    if (!pProcInfo)
    {
        pProcInfo = FindProcInArrayByPID((CPtrArray *)m_pProcArray, pid);
    }
    if (!pProcInfo)
    {
        return FALSE;
...
HRESULT CProcPage::Initialize(HWND hwndParent)
{
    ...
    // ???????? why the fuck is msvc so weird about these god damn types
    *(CPtrArray **)&m_pProcArray = new CPtrArray(GetProcessHeap());
