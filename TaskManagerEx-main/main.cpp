...

{
    ...
    status = CallNtPowerInformation(SystemPowerCapabilities, NULL, 0, &spc, sizeof(spc));
    if ( NOERROR == status )
    {
...

{
    ...
    if (fFound)
    {
        ...
        fHasShutdownPrivilege = (SHTestTokenPrivilegeW(NULL, SE_SHUTDOWN_NAME) != FALSE);
        ...
        if (!fIsDocked || (SHTestTokenPrivilegeW(NULL, SE_UNDOCK_NAME) == FALSE) || (gpfnCM_Request_Eject_PC == NULL))
        {
...
void IsTerminalServicesEnabled( LPBOOL pfIsTSEnabled, LPBOOL pfIsSingleUserTS, LPBOOL pfIsTSServer  )
{
    ...
    *pfIsTSServer = FALSE;
    ...
    ZeroMemory(&osVersionInfo, sizeof(RTL_OSVERSIONINFOEXW));
    osVersionInfo.dwOSVersionInfoSize = sizeof(RTL_OSVERSIONINFOEXW);

    if(NT_SUCCESS(RtlGetVersion((RTL_OSVERSIONINFOW*)&osVersionInfo)))
    {
